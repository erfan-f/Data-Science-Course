...
class GlotMF:
    ...
    def train(self, ratings_df, reputation_weights, user_id_to_idx, item_id_to_idx, trust_neighbors, S_B, S_E, epochs=10):
...
class GlotMFPreprocessor:
    ...
    def build_rating_matrix(self):
        rows = self.ratings_df['user_id'].map(self.user_id_to_idx)
        cols = self.ratings_df['item_id'].map(self.item_id_to_idx)
        data = self.ratings_df['label'].astype(float)
    ...
    def build_trust_matrix(self):
        rows = self.trust_df['user_id_trustor'].map(self.user_id_to_idx)
        cols = self.trust_df['user_id_trustee'].map(self.user_id_to_idx)
        data = self.trust_df['trust_value'].astype(float)

        trust_matrix = csr_matrix((data, (rows, cols)),
